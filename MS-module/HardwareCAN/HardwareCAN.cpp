 ...
 */
	...
	if (can_init(Port,CAN_MCR_TXFP, speed) == CAN_OK)
 // End Fix JMD
...
CAN_STATUS HardwareCAN::filter(uint8 idx, CAN_FIFO fifo_, uint32 id, uint32 mask)
{
...
{
	return begin(CAN_SPEED_95, CAN_MODE_NORMAL);
}
