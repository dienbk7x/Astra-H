...
void setup() {
  ...
  pinMode(BLUEPIN, PWM); // input for hardware button
  pinMode(REDPIN, PWM); // input for hardware switch

  Serial1.begin(115200);
  Serial1.println("Hello World!");
  Serial1.println("Starting rpm-to-LED program");
  Serial1.println("Initialize the CAN module ...");

  CANSetup() ;        // Initialize the CAN module and prepare the message structures.
     Serial1.print("Initialization OK");
  rpm=redvalue=bluevalue=0;
...
void loop() {
    ...
    while ( ( r_msg = canBus.recv() ) != NULL )
    {
      ...
      if (rpm < 1001)
        {
          redvalue = 0;
          bluevalue = map(500, 0, 1000, 0, 65535);
        }
      else if (rpm < 2001)
        {
          redvalue = 0;
          bluevalue = map(rpm, 0000, 2000, 0, 65535);
        }
      else if (rpm < 3001)
        {
          redvalue = map(rpm, 2000, 3000, 0, 65535);
          bluevalue = 65535;
        }
      else if (rpm < 4001)
        {
          redvalue = 65535;
          bluevalue = map(rpm, 3001, 4000, 65535, 0);
        }
      else
        {
          redvalue = 65535;
      ...
      pwmWrite(BLUEPIN, bluevalue);
      pwmWrite(REDPIN, redvalue);
      }
    canBus.free();
	  }     
}
