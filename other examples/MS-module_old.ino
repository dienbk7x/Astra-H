...
void setup() {        // Initialize the CAN module and prepare the message structures.
	...
	usbMode = digitalRead(28); // if B12 is HIGH, then switch to programming mode
	if (usbMode)
	{
	...
	else
	{
		...
		blocked=false;
		CAN_b_95_Setup();
		SendCANmessage(0x201,3,0x01,0xFF,0x00); // press settings      
		delay(50);
		SendCANmessage(0x201,3,0x00,0xFF,0x00); // release settings      
		delay(50);
...
/**/
	CAN_b_95_Setup();
//	Serial2.println("started CAN 95");
	// read MS CAN and record data into variables (or make instant action)
/* start read MS CAN */
	 // CanMsg *r_msg; /// can this be moved out of "loop"? already there??
	//	r_msg = canBus.recv();
		
		Serial2.println((r_msg = canBus.recv()) != NULL );
		...
		case 0x206: // steering wheel buttons
			...
			if (r_msg->Data[1]==0x82) 	// "aux/hands free" button
			{
...
/**/		
		...
		switch (r_msg->Data[0]) // mode? 
		{
			...
			break;
		} // close switch (r_msg->Data[0]) 
		break; // case 0x6C8: // ID = climate info
			
		default:
		break;
		} // close switch(r_msg->ID)
		/// one word read. can go back and repeat loop N times, but then need to have global variables?
		}
...
///// actions /////		
		if (climChanged) 
		{
			Serial2.println("Change of climate control detected, sending new data to odometer");
			digitalWrite(PC13,PC13ON);
			delay(50);
			digitalWrite(PC13,PC13OFF);
			delay(50);
